...
- **Solar+Battery mode now works as described (v1.13.10, v1.13.11).** The mode is meant to charge the car from solar and cover any shortfall from the house battery, but two separate faults meant it never did. It checked the battery against the *arbitrage export reserve* — a floor commonly sitting at 55-70 % once the dynamic reserve is active — rather than the inverter's own minimum, so on an ordinary day it refused to use the battery at all and behaved exactly like plain PV mode. Even when it did engage, it asked for only the single-phase minimum (~1.4 kW), and the phase-upshift decision reads a solar-only signal that deliberately excludes battery power, so it could never reach three-phase. Solar+Battery now targets the 4.14 kW three-phase minimum whenever the battery is above the inverter's own Min-SoC, with the battery supplying only the gap solar doesn't cover and solar tracking 1:1 above it. When the battery reaches that floor it stops contributing and the car continues on solar alone. Scoped to Solar+Battery only — plain PV, Full, and Locked are unchanged.
- **Three-phase PV mode could drain the house battery when export was price-blocked (v1.13.9).** With the battery near full and export blocked, the inverter's PV-limited flag held two independent gates open — the curtailment override's three-phase hold and the dip-bridge's fall-back suppression — so the car sat at the three-phase floor while solar dipped below it and the house battery quietly covered the difference, with grid import reading near zero throughout. Observed as roughly 5 % of house battery over half an hour. Both gates now yield once the battery is genuinely supplying the deficit.
- **Cool-down countdown shows the real stop time and ticks every second (v1.13.8).** The "EV stopping in X s" banner counted down from the full stop window regardless of whether the shorter dip-bridge dwell was going to end the session first, and only refreshed on each control cycle. It now shows whichever limit actually applies and counts down smoothly.
